...
import { Lock, Sparkles, Building2, ShieldCheck, ArrowRight } from 'lucide-react';
...
export default function AdminPage() {
  ...
  useEffect(() => {
    // Check saved session
    const authSession = sessionStorage.getItem('ssp_standalone_admin_auth');
    if (authSession === 'true') {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-[#f0f4f8] text-slate-800 flex flex-col justify-center items-center px-4 relative overflow-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
        {/* Ambient Radial Glow */}
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f8] text-slate-800 flex" style={{ fontFamily: "'Inter', sans-serif" }}>
      {/* Sidebar */}
      <Sidebar
        activeTab={activeTab}
