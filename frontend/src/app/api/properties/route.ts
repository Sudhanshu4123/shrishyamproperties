  ...
  try {
    ...
    if (purpose && purpose !== 'All') {
      ...
      list = list.filter((p: any) => 
        (p.purpose && p.purpose.toLowerCase() === purp) ||
        (purp === 'commercial' && (p.type?.toLowerCase() === 'commercial' || p.type?.toLowerCase() === 'shop'))
      );
    }

    if (type && type !== 'All') {
      list = list.filter((p: any) => p.type?.toLowerCase() === type.toLowerCase());
    }

    if (sector && sector !== 'All' && sector !== 'All Locations') {
      list = list.filter((p: any) => p.sector?.toLowerCase() === sector.toLowerCase());
    }

    if (bhk && Number(bhk) > 0) {
      list = list.filter((p: any) => Number(p.bhk) === Number(bhk));
    }

    if (minPrice) {
      list = list.filter((p: any) => Number(p.priceValue) >= Number(minPrice));
    }

    if (maxPrice) {
      list = list.filter((p: any) => Number(p.priceValue) <= Number(maxPrice));
    }
