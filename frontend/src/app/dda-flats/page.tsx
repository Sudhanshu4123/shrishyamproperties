...
import { Building2, CheckCircle2, ShieldCheck, Box, X, Trees } from 'lucide-react';

export default function DdaFlatsPage() {
  ...
  useEffect(() => {
    PropertyService.fetchPropertiesApi({ type: 'DDA Flat' }).then((results) => {
      const ddaProps = results.filter((p) => p.type === 'DDA Flat' || p.type === 'Society Flat');
      setProperties(ddaProps);
      setLoading(false);
    });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f8] text-slate-800 flex flex-col font-sans">
      ...
      <main className="pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full flex-1">
        <Breadcrumbs items={[{ label: 'DDA Flats in Dwarka' }]} />

        {/* Hero Banner */}
        <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200 shadow-sm mb-10">
          <span className="section-label block mb-2">Government Planned Layouts</span>
          <h1
            className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight mb-4"
            style={{ fontFamily: "'Playfair Display', serif" }}
          >
            DDA Flats & Apartments in Dwarka, New Delhi
          </h1>
          <p className="text-sm sm:text-base text-slate-600 max-w-3xl leading-relaxed mb-6">
            Find verified DDA MIG, HIG, and SFS apartments across Dwarka Sector 6, Sector 8, Sector 11, Sector 13, and Sector 19. Enjoy vast open green pockets, central community parks, wide internal roads, and seamless walking access to Delhi Metro stations. All flats verified with original DDA Conveyance Deeds.
          </p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-6 border-t border-slate-100 text-xs font-semibold text-slate-700">
            <div className="flex items-center gap-2 text-teal-700">
              ...
              <span>DDA Freehold Conveyance Deed</span>
            </div>
            <div className="flex items-center gap-2 text-teal-700">
              <CheckCircle2 className="w-4 h-4 text-teal-600" />
              <span>Extensive Green Parks</span>
            </div>
            <div className="flex items-center gap-2 text-teal-700">
              <CheckCircle2 className="w-4 h-4 text-teal-600" />
              <span>Bank Loan Approved (SBI/HDFC)</span>
            </div>
            <div className="flex items-center gap-2 text-teal-700">
              <CheckCircle2 className="w-4 h-4 text-teal-600" />
              <span>Low Maintenance Gated Pockets</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-xl font-bold text-slate-900">
            Available DDA & Society Flats ({properties.length})
          </h2>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-3xl border border-slate-200 shadow-sm">
            <Trees className="w-12 h-12 text-teal-500/40 mx-auto mb-3" />
            <h3 className="text-lg font-bold text-slate-800">DDA Flat Listings Updating</h3>
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              We have fresh DDA MIG & HIG pocket inventory in Sectors 6, 8, 10, and 19. Call our desk at +91 9911956274.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
